...
export default function GeneratePage() {
      ...
      try {
        const { data, error } = await supabase.functions.invoke('generate-content', {
          body: {
            ...
            platform,
            category,
            custom_prompt: `Entite : ${entity}. Sujet : ${topic}`,
          },
        })

        if (error) {
          results[platform] = `[Erreur] ${error.message}`
        } else if (data?.content) {
          results[platform] = data.content
        } else {
          results[platform] = '[Aucun contenu genere]'
        }
      } catch {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-outfit text-2xl font-bold text-offwhite">Generateur de contenu</h1>
        ...
        <div className="bg-engine-surface border border-engine-border rounded-xl p-5 space-y-5">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Decrivez le sujet du post..."
              className="w-full bg-engine-bg border border-engine-border rounded-lg px-3 py-2 text-sm text-offwhite placeholder-gray-600 focus:outline-none focus:border-teal resize-none h-24"
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="bg-engine-bg border border-engine-border text-offwhite rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-teal"
            >
              {CATEGORIES.map((c) => (
          ...
          <div className="flex gap-3">
            <div className="flex-1">
              ...
              <input
                ...
                className="w-full bg-engine-bg border border-engine-border rounded-lg px-3 py-2 text-sm text-offwhite focus:outline-none focus:border-teal"
              />
            </div>
            <div className="w-28">
          ...
          <button
            ...
            className="w-full py-2.5 bg-teal hover:bg-teal/80 disabled:bg-gray-700 disabled:text-gray-500 text-white rounded-lg text-sm font-semibold transition-colors"
          >
        ...
        <div className="space-y-4">
          {Object.keys(generatedContent).length === 0 && !generating && (
            <div className="bg-engine-surface border border-engine-border rounded-xl p-8 text-center">
              <p className="text-gray-500 text-sm">Le contenu genere apparaitra ici</p>
            </div>
          )}
          ...
          {generating && (
            <div className="bg-engine-surface border border-engine-border rounded-xl p-8 text-center">
              ...
              <p className="text-gray-400 text-sm">Generation IA en cours...</p>
            </div>
          )}
          ...
          {Object.keys(generatedContent).length > 0 && !scheduled && (
            <button
              ...
              className="w-full py-2.5 bg-gold hover:bg-gold/80 disabled:bg-gray-700 disabled:text-gray-500 text-ink rounded-lg text-sm font-semibold transition-colors"
            >
          ...
          {scheduled && (
            <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-4 text-center">
              <p className="text-emerald-400 text-sm font-medium">Posts planifies avec succes !</p>
              <p className="text-emerald-400/60 text-xs mt-1">Visible dans la queue de publication</p>
            </div>
          )}
        </div>
      </div>
