...
export default function WorkflowsPage() {
  ...
  function getLastRun(workflowId: string): WorkflowRun | null {
    return runs[workflowId]?.[0] || null
  }

  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-outfit text-2xl font-bold text-offwhite">Workflows</h1>
      ...
      {loading ? (
        ...
        <div className="bg-engine-surface border border-engine-border rounded-xl p-8 text-center">
          <p className="text-gray-400">Aucun workflow configure</p>
          <p className="text-gray-500 text-xs mt-2">Les workflows seront crees automatiquement lors du deploiement des Edge Functions</p>
        </div>
      ) : (
        <div className="space-y-3">
          {workflows.map((wf) => {
            ...
            return (
              <div
                key={wf.id}
                className="bg-engine-surface border border-engine-border rounded-xl overflow-hidden"
              >
                {/* Workflow header */}
                <div
                ...
                >
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation()
                        toggleWorkflow(wf.id, wf.is_active)
                      }}
                      className={`w-10 h-5 rounded-full relative transition-colors ${
                        wf.is_active ? 'bg-teal' : 'bg-gray-600'
                      }`}
                    >
                      <span
                        className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform ${
                          wf.is_active ? 'left-5' : 'left-0.5'
                        }`}
                      />
                    </button>

                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-offwhite text-sm">{wf.name}</p>
                      <p className="text-xs text-gray-500 mt-0.5">
                        {TYPE_LABELS[wf.type] || wf.type}
                        {wf.cron_schedule && (
                          <span className="ml-2 font-mono text-gray-600">({wf.cron_schedule})</span>
                        )}
                      </p>
                    </div>
                  </div>
                    ...
                    {lastRun && (
                      <div className="text-right">
                        <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${
                          RUN_STATUS_STYLES[lastRun.status]?.bg || ''
                        } ${RUN_STATUS_STYLES[lastRun.status]?.text || ''}`}>
                          {lastRun.status}
                        </span>
                        <p className="text-[10px] text-gray-500 mt-0.5">{formatDate(lastRun.created_at)}</p>
                      </div>
                    )}
                ...
                {isExpanded && (
                  <div className="border-t border-engine-border">
                    <div className="p-3 bg-black/20">
                      ...
                      {wfRuns.length === 0 ? (
                        <p className="text-xs text-gray-600 py-2">Aucune execution</p>
                      ) : (
                          ...
                          {wfRuns.map((run) => (
                            ...
                            >
                              <span className={`inline-flex px-1.5 py-0.5 rounded text-[10px] font-medium ${
                                RUN_STATUS_STYLES[run.status]?.bg || ''
                              } ${RUN_STATUS_STYLES[run.status]?.text || ''}`}>
                                {run.status}
                              </span>
                              <span className="text-gray-500">{formatDate(run.created_at)}</span>
                              {run.duration_ms && (
                                <span className="text-gray-600">{run.duration_ms}ms</span>
                              )}
                              {run.error_message && (
                                <span className="text-red-400/70 truncate max-w-[300px]">
                                  {run.error_message}
                                </span>
                              )}
                            </div>
